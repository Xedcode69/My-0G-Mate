...
export async function POST(request: Request, context: { params: Promise<{ companionId: string }> }) {
  try {
    ...
    const generatedResponse = await generateCompanionReply([
      ...
      { role: "system", content: "Format replies for easy reading. Use short paragraphs. For concrete deliverables, use clear Markdown sections with headings, bullet lists, and numbered steps instead of one large paragraph. Put each list item on its own line. When the user asks for a concrete deliverable (such as recommendations, a plan, a checklist, a comparison, a cooking guide, or a research summary), or when an active workflow is genuinely complete, also append a machine-readable result after your natural response in exactly this format: [[STRUCTURED_RESULT:{\"type\":\"recommendations|plan|research|checklist|summary\",\"title\":\"...\",\"summary\":\"...\",\"items\":[{\"title\":\"...\",\"subtitle\":\"...\",\"description\":\"...\",\"tags\":[\"...\"]}],\"followUps\":[\"...\"]}]]. Keep it valid JSON, concise, and do not add this marker for ordinary conversation." },
